...
void Modem::recv(void) {
	...
	// Stop bit reception
		...
		if (high) {
			...
			uint8_t new_tail =
					(_recvBufferTail + 1 == rxBufSize) ?
							0 : _recvBufferTail + 1;
			if (new_tail != _recvBufferHead) {
				*(_recvBuffer + _recvBufferTail) = _recvBits;
				_recvBufferTail = new_tail;
			} else {
				//TODO: Overrun error detection
			}
		} else {
			//TODO: Fleming error detection
		}
		goto end_recv;
	} else {
		end_recv: _recvStat = INACTIVE;
		TIMSK &= ~(1 << OCIE1A);
	}
}
...
uint8_t Modem::read() {
	...
	_recvBufferHead =
			(_recvBufferHead + 1 == rxBufSize) ? 0 : _recvBufferHead + 1;
	return c;
}
